...
#headers to install
headers = glob.glob(os.path.join('pygame-1.9.6/src_c', '*.h'))
headers.remove(os.path.join('pygame-1.9.6/src_c', 'scale.h'))
...
try:
    ...
    sin_mtime = os.stat(os.path.join('pygame-1.9.6/buildconfig', 'Setup.SDL1.in'))[stat.ST_MTIME]
...
alternate_font = os.path.join('pygame-1.9.6/src_py', 'font.py')
...
if not have_font and have_freetype:
    shutil.copyfile(os.path.join('pygame-1.9.6/src_py', 'ftfont.py'), alternate_font)
...
for f in glob.glob(os.path.join('pygame-1.9.6/src_py', '*')):
...
def parse_source_version():
    ...
    pg_header = os.path.join('pygame-1.9.6/src_c', '_pygame.h')
...
def write_version_module(pygame_version, revision):
    ...
    with open(os.path.join('pygame-1.9.6/buildconfig', 'version.py.in'), 'r') as header_file:
        header = header_file.read()
    with open(os.path.join('pygame-1.9.6/src_py', 'version.py'), 'w') as version_file:
            ...
            if e.name == 'transform':
                if '64 bit' in sys.version:
                    e.extra_objects.append(
                        os.path.join('pygame-1.9.6/buildconfig', 'obj', 'win64', 'scale_mmx.obj'))
                else:
                    e.extra_objects.append(
                        os.path.join('pygame-1.9.6/buildconfig', 'obj', 'win32', 'scale_mmx.obj'))
...
class OurSdist(sdist):
    def initialize_options(self):
        ...
        self.template = os.path.join('pygame-1.9.6/buildconfig', 'MANIFEST.in')
...
class TestCommand(Command):
    ...
    def run(self):
        ...
        '''
        import subprocess
        return subprocess.call([sys.executable, os.path.join('pygame-1.9.6/test', '__main__.py')])
    ...
    def run(self):
        ...
        try:
            return subprocess.call([
                sys.executable, os.path.join('pygame-1.9.6/buildconfig', 'makeref.py')]
            )
        except:
            print(docs_help)
            raise
